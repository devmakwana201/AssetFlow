 ...
 *
 *   https://app.assetflow.app/qr/{id}
 *   https://app.assetflow.app/assets/{id}
 *   https://app.assetflow.app/bookings/{id}
 *   https://app.assetflow.app/audits/{id}
 *
...
function parseDeepLink(url: string): ParsedLink {
  try {
    ...
    // (https://app.assetflow.app/kits/abc): for the custom scheme expo-linking puts
...
async function resolveQrAndNavigate(qrId: string) {
  ...
  void openShelfWebUrl(`https://app.assetflow.app/qr/${qrId}`);
}
