...
This guide provides comprehensive instructions for developers on how to add new barcode types to the AssetFlow application. Due to the distributed nature of barcode handling across multiple layers of the application, this guide ensures all necessary changes are made systematically.

## Overview

The barcode system in AssetFlow is implemented across multiple layers:
...
```bash
pnpm --filter @assetflow/webapp exec prisma migrate dev --name add-new-barcode-type
```
...
**File:** `public/static/assetflow.app-example-asset-import-from-content-with-barcodes.csv`
