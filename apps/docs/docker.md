...
docker run -d \
  ...
  -e 'SMTP_FROM="Your Name from assetflow.app" <your-email@assetflow.app>' \
  -e 'SMTP_PWD=super-safe-passw0rd' \
  -e 'INVITE_TOKEN_SECRET=another-super-duper-s3cret' \
  -p 3000:8080 \
  --restart unless-stopped \
  ghcr.io/assetflow/assetflow.app:latest
```
...
1. Linux / Pine A64
   ...
   ```bash
   docker run -it --rm --entrypoint /usr/bin/uname ghcr.io/assetflow/assetflow.app:latest -a
...
2. MacOS / M1 Max
   ...
   ```bash
   docker run -it --rm --platform linux/arm64 --entrypoint /usr/bin/uname ghcr.io/assetflow/assetflow.app:latest -a
   # Expected output: Linux ... aarch64 GNU/Linux
   ```
