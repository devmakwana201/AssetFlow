...
This guide covers deploying AssetFlow to production environments. We'll focus on Fly.io deployment with GitHub Actions for CI/CD.
...
- ✅ **GitHub repository** with your AssetFlow code
...
fly apps create assetflow-webapp

# Staging app (optional but recommended)
fly apps create assetflow-webapp-staging
```
...
fly secrets set SMTP_FROM="AssetFlow <noreply@yourapp.com>"
...
```bash
# Repeat similar commands with --app assetflow-webapp-staging
fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app assetflow-webapp-staging
fly secrets set SERVER_URL="https://your-staging-app.fly.dev" --app assetflow-webapp-staging
...
fly deploy --app assetflow-webapp-staging
```
...
3. **Configure SMTP**:
   ...
   fly secrets set SMTP_FROM="AssetFlow <noreply@yourdomain.com>"
   ```
...
- 🐛 **[GitHub Issues](https://github.com/AssetFlow/assetflow.app/issues)** - Report deployment issues
- 📧 **[Fly.io Support](https://fly.io/docs/getting-help/)** - Fly.io specific support

Congratulations on deploying AssetFlow! 🎉
