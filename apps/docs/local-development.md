...
This guide covers everything you need to know for developing AssetFlow locally after completing the [Supabase Setup](./supabase-setup.md).
...
AssetFlow is organized as a **pnpm + Turborepo monorepo**. All commands use `pnpm` instead of `npm`.

| Package                    | Path                  | Description                            |
| -------------------------- | --------------------- | -------------------------------------- |
| `@assetflow/webapp`            | `apps/webapp/`        | Remix web application                  |
| `@assetflow/docs`              | `apps/docs/`          | VitePress documentation site           |
| `@assetflow/database`          | `packages/database/`  | Prisma client factory and shared types |
| `@assetflow/typescript-config` | `tooling/typescript/` | Shared TypeScript configurations       |
...
git clone https://github.com/AssetFlow/assetflow.app.git
cd assetflow.app
...
pnpm --filter @assetflow/webapp test:e2e         # Run end-to-end tests
pnpm --filter @assetflow/webapp test:e2e:dev     # Run E2E tests in dev mode
pnpm --filter @assetflow/webapp test:e2e:install # Install Playwright browsers
```
...
├── apps/
│   ├── webapp/                      # @assetflow/webapp — Remix app
│   │   ├── app/
│   │   │   ├── components/          # Reusable UI components
│   │   │   ├── database/            # DB client (re-exports @assetflow/database)
│   │   │   ├── modules/             # Business logic modules
│   │   │   ├── routes/              # Remix routes (pages)
│   │   │   ├── utils/               # Utility functions
│   │   │   └── root.tsx             # App root component
│   │   └── package.json
│   └── docs/                        # @assetflow/docs — VitePress documentation
├── packages/
│   └── database/                    # @assetflow/database — Prisma client + types
│       ├── prisma/
...
```bash
pnpm --filter @assetflow/webapp exec prisma studio
```
...
```bash
pnpm --filter @assetflow/webapp test:e2e:install  # Install browsers (first time)
pnpm --filter @assetflow/webapp test:e2e:dev      # Run tests in development
```
...
```bash
pnpm --filter @assetflow/webapp exec prisma studio  # Visual database browser
```
...
- 🐛 **[GitHub Issues](https://github.com/AssetFlow/assetflow.app/issues)** - Report bugs or request features
- 🐦 **[Twitter](https://twitter.com/AssetFlow)** - Follow for updates
