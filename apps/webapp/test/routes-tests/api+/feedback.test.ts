  ...
  describe("action", () => {
    ...
    it("should redact secret-looking query params from the page URL", async () => {
      const request = createFeedbackRequest({
        ...
        message: "Something is broken in the app",
        currentUrl:
          "https://app.assetflow.app/reset-password?token=supersecret&search=laptop",
      });
      ...
      expect(sendFeedbackEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          currentUrl:
            "https://app.assetflow.app/reset-password?token=redacted&search=laptop",
        })
      );
    });
    ...
    it("should forward the auto-captured context and error details", async () => {
      const request = createFeedbackRequest({
        type: "issue",
        message: "Something is broken in the app",
        currentUrl: "https://app.assetflow.app/kits",
