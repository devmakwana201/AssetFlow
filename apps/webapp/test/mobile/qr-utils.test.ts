...
describe("extractQrId", () => {
  ...
  describe("full URL format: https://<domain>/qr/<id>", () => {
    it("should extract ID from standard Shelf URL", () => {
      expect(extractQrId("https://app.assetflow.app/qr/abc123def456")).toBe(
        "abc123def456"
      );
    });

    it("should extract mixed-case ID from full URL", () => {
      expect(extractQrId("https://assetflow.app/qr/XyZ789")).toBe("XyZ789");
    });
    ...
    it("should handle subdomains", () => {
      expect(extractQrId("https://staging.assetflow.app/qr/abcdef1234")).toBe(
        "abcdef1234"
      );
    });
  });
  ...
  describe("URL shortener format: https://<domain>/<id>", () => {
    it("should extract 10-char shortener ID", () => {
      expect(extractQrId("https://s.assetflow.app/abcdefghij")).toBe("abcdefghij");
    });

    it("should extract 25-char shortener ID", () => {
      expect(extractQrId("https://s.assetflow.app/abcdefghijklmnopqrstuvwxy")).toBe(
  ...
  describe("edge cases that should return null", () => {
    ...
    it("should reject URL with additional path segments after QR ID", () => {
      expect(extractQrId("https://assetflow.app/qr/abc123/extra")).toBeNull();
    });
