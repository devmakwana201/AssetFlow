...
export async function sendAuditAssignedEmail({
  ...
  try {
    ...
    sendEmail({
      to: assigneeEmail,
      subject: `🔍 You've been assigned to audit: "${audit.name}" - assetflow.app`,
  ...
  assigneesToNotify.forEach(async (assignment) => {
    try {
      ...
      sendEmail({
        ...
        subject: `❌ Audit cancelled: "${audit.name}" - assetflow.app`,
...
export function sendAuditCompletedEmail({
  ...
  assigneesToNotify.forEach(async (assignment) => {
    try {
      ...
      sendEmail({
        to: assignment.user.email,
        subject: `✅ Audit completed: "${audit.name}" - assetflow.app`,
...
export function sendAuditReminderEmail({
  ...
  assignees.forEach(async (assignment) => {
    try {
      ...
      sendEmail({
        to: assignment.user.email,
        subject: `${heading}: "${audit.name}" - assetflow.app`,
...
export function sendAuditOverdueEmail({
  ...
  recipients.forEach(async (recipient) => {
    try {
      ...
      sendEmail({
        to: recipient.email,
        subject: `⚠️ Audit overdue: "${audit.name}" - assetflow.app`,
