...
describe("buildBookingVEvent", () => {
  const baseInput = {
    ...
    bookingUrl: "https://app.assetflow.app/bookings/booking-123",
  ...
  it("emits a well-formed VEVENT with summary, dates, description and alarm", () => {
    ...
    expect(description).toContain(
      "View booking: https://app.assetflow.app/bookings/booking-123"
    );
  });
...
describe("buildBookingICalendar", () => {
  const vevent = buildBookingVEvent({
    ...
    custodianName: "Sam",
    assetTitles: ["Lens"],
    bookingUrl: "https://app.assetflow.app/bookings/b1",
  ...
  it("wraps events in a VCALENDAR envelope with CRLF line endings", () => {
    ...
    expect(ics).toContain("PRODID:-//AssetFlow//Shelf Calendar 1.0//EN");
  ...
  it("includes every event in a multi-event feed", () => {
    const second = buildBookingVEvent({
      ...
      custodianName: "Pat",
      assetTitles: [],
      bookingUrl: "https://app.assetflow.app/bookings/b2",
  ...
  it("folds long lines so every line stays within 75 octets", () => {
    const longVevent = buildBookingVEvent({
      ...
      custodianName: "Q",
      assetTitles: [],
      bookingUrl: "https://app.assetflow.app/bookings/b3",
