...
// why: env vars are read at import time; assetflow.config.ts also imports from this module
vi.mock("~/utils/env", () => ({
  SERVER_URL: "https://app.assetflow.app",
  SUPPORT_EMAIL: "support@assetflow.app",
...
const CONTEXT_PROPS = {
  ...
  currentUrl: "https://app.assetflow.app/assets",
  userAgent:
...
describe("feedbackEmailText", () => {
  ...
  it("includes the auto-captured context rows when provided", () => {
    ...
    expect(text).toContain("Page: https://app.assetflow.app/assets");
...
describe("feedbackEmailHtml", () => {
  it("links the page URL and renders error details", async () => {
    ...
    expect(html).toContain('href="https://app.assetflow.app/assets"');
  ...
  it("is not fooled by prefix-matching hosts or userinfo tricks", async () => {
    ...
    for (const url of [
      "https://app.assetflow.app.evil.com/phish",
      "https://app.assetflow.app@evil.com/phish",
      "not a url at all",
    ]) {
