...
describe("hostFromUrl", () => {
  it("returns the lowercased host (with port if present)", () => {
    expect(hostFromUrl("https://app.assetflow.app")).toBe("app.assetflow.app");
    expect(hostFromUrl("https://APP.Shelf.NU/login")).toBe("app.assetflow.app");
...
describe("securityHeaders middleware", () => {
  ...
  beforeEach(() => {
    process.env.SERVER_URL = "https://app.assetflow.app";
  });
  ...
  it("sets headers on a normal dynamic response", async () => {
    const res = await makeApp().request("https://app.assetflow.app/login");
  ...
  it("sets headers on a short-circuiting static-like response", async () => {
    const res = await makeApp().request("https://app.assetflow.app/static/app.js");
