...
This repository hosts **AssetFlow**, an asset management platform built with Remix, React, TypeScript, and PostgreSQL. This is a **pnpm + Turborepo monorepo**. Use `pnpm` instead of `npm`. Follow the instructions below when working anywhere in this repository.

## Monorepo Structure

- `apps/webapp/` — `@assetflow/webapp` — Remix web application
- `packages/database/` — `@assetflow/database` — Prisma schema, migrations, and client factory
- `tooling/typescript/` — `@assetflow/typescript-config` — Shared tsconfig bases

All database concerns (schema, migrations, Prisma client generation) are owned by `@assetflow/database`. The webapp consumes it as a workspace dependency via `apps/webapp/app/database/db.server.ts`.
...
All database commands run via the `@assetflow/database` package:
...
- `pnpm --filter @assetflow/webapp lint:fix` – Fix ESLint issues automatically.
...
- The webapp's `apps/webapp/app/database/db.server.ts` is a thin wrapper around `@assetflow/database`'s `createDatabaseClient()` factory.
